...
clear
msg -bar
rm -rf /etc/VPS-BO/demo-ssh 2>/dev/null
mkdir /etc/VPS-BO/demo-ssh 2>/dev/null
SCPdir="/etc/VPS-BO"
...
rm -rf /tmp/cmd
touch /etc/VPS-BO/demo-ssh/$name
echo "senha: $pass" >> /etc/VPS-BO/demo-ssh/$name
echo "data: ($tmp)Minutos" >> /etc/VPS-BO/demo-ssh/$name
msg -bar2
echo -e "\033[1;93m ¡¡ USUARIO TEMPORAL x MINUTOS (VPS-BO By @Kalix1) !!\033[0m"
msg -bar2
