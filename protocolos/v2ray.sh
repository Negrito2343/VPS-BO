...
USRdatabase="/etc/VPS-BO/RegV2ray"
...
rm -rf /etc/VPS-BO/RegV2ray > /dev/null 2>&1
...
##Registro
echo "  $UUID | $nick | $valid " >> /etc/VPS-BO/RegV2ray
Fecha=`date +%d-%m-%y-%R`
cp /etc/VPS-BO/RegV2ray /etc/VPS-BO/v2ray/RegV2ray-"$Fecha"
cp /etc/v2ray/config.json /etc/VPS-BO/v2ray/config.json-"$Fecha" 
v2ray restart > /dev/null 2>&1
echo ""
v2ray info > /etc/VPS-BO/v2ray/confuuid.log
lineP=$(sed -n '/'${UUID}'/=' /etc/VPS-BO/v2ray/confuuid.log)
numl1=4
let suma=$lineP+$numl1
sed -n ${suma}p /etc/VPS-BO/v2ray/confuuid.log 
echo ""
msg -bar
...
echo -e "\e[33m$(cat /etc/VPS-BO/RegV2ray|cut -d '|' -f2,1)" 
msg -bar
...
linePre=$(sed -n '/'${uuidel}'/=' /etc/VPS-BO/RegV2ray)
sed -i "${linePre}d" /etc/VPS-BO/RegV2ray
numl1=2
...
msg -bar
# usersss=$(cat /etc/VPS-BO/RegV2ray|cut -d '|' -f1)
# cat /etc/VPS-BO/RegV2ray|cut -d'|' -f3
VPSsec=$(date +%s)
local HOST="/etc/VPS-BO/RegV2ray"
local HOST2="/etc/VPS-BO/RegV2ray"
...
DateExp="$(cat /etc/VPS-BO/RegV2ray|grep -w "$hostreturn"|cut -d'|' -f3)"
if [[ ! -z $DateExp ]]; then             
...
usris="$(cat /etc/VPS-BO/RegV2ray|grep -w "$hostreturn"|cut -d'|' -f2)"
...
linesss=$(cat /etc/VPS-BO/RegV2ray | wc -l)
...
estarts () {
VPSsec=$(date +%s)
local HOST="/etc/VPS-BO/v2ray/lisportt.log"
local HOST2="/etc/VPS-BO/v2ray/lisportt.log"
local RETURN="$(cat $HOST|cut -d'|' -f2)"
local IDEUUID="$(cat $HOST|cut -d'|' -f1)"
if [[ -z $RETURN ]]; then
...
i=1
while read hostreturn ; do
iptables -n -v -L > /etc/VPS-BO/v2ray/data1.log 
statsss=$(cat /etc/VPS-BO/v2ray/data1.log|grep -w "tcp spt:$hostreturn quota:"|cut -d' ' -f3,4,5)
gblim=$(cat /etc/VPS-BO/v2ray/lisportt.log|grep -w "$hostreturn"|cut -d'|' -f2)
...
[[ ! -z $contador_secuencial ]] && {
linesss=$(cat /etc/VPS-BO/v2ray/lisportt.log | wc -l)
...
echo ""
echo " $portbg | $gbuser | $multiplicacion " >> /etc/VPS-BO/v2ray/lisportt.log 
msg -bar
msg -ne "Enter Para Continuar" && read enter
${SCPinst}/v2ray.sh
}
...
[[ $(sed -n '/'${portbg}'/=' /etc/VPS-BO/v2ray/lisportt.log|head -1) ]] || invaliduuid
gblim=$(cat /etc/VPS-BO/v2ray/lisportt.log|grep -w "$portbg"|cut -d'|' -f3)
sudo iptables -D OUTPUT -p tcp --sport $portbg -j DROP
sudo iptables -D OUTPUT -p tcp --sport $portbg -m quota --quota $gblim -j ACCEPT
iptables-save > /etc/iptables/rules.v4
lineP=$(sed -n '/'${portbg}'/=' /etc/VPS-BO/v2ray/lisportt.log)
sed -i "${linePre}d" /etc/VPS-BO/v2ray/lisportt.log
msg -bar
msg -ne "Enter Para Continuar" && read enter
${SCPinst}/v2ray.sh 
}
